% ----------------------------------------------------------------------- %
% Anti-Coronavirus Optimization (ACVO) for unconstrained benchmark problems
% ----------------------------------------------------------------------- %
...
function [pop_hist, fit_hist, hist_idx, curve] = record_block(FE_before, FE, maxFE, bestcost, pop, popSize, dim, curve, pop_hist, fit_hist, hist_idx, sampling_interval, history_size)
    np = numel(pop);
    take = min(popSize, np);
...
function [pop, I, FE, bf, bs] = isolation(ProblemParams, AlgorithmParams, pop, Q, I, bestSolution, problem, FE, bf, bs) %#ok<INUSD>
    ...
    for i = 1:numel(I)
        if ((I(i).status == -1) && (I(i).days <= AlgorithmParams.isolationDuration))
            ...
            I(i).days = I(i).days + 1;
            [I(i).cost, FE, bf, bs] = evalp(I(i).position, problem, FE, bf, bs);
        else
            [I(i).cost, FE, bf, bs] = evalp(I(i).position, problem, FE, bf, bs);
            if (I(i).cost <= I(i).fs)
                I(i).status = 1;
...
function [pop, Q, I, FE, bf, bs] = quarantine(ProblemParams, AlgorithmParams, pop, Q, I, bestSolution, problem, FE, bf, bs) %#ok<INUSD>
    ...
    for i = 1:numel(Q)
        if ((Q(i).status == 0) && (Q(i).days < AlgorithmParams.quarantineDuration))
            ...
            [Q(i).cost, FE, bf, bs] = evalp(Q(i).position, problem, FE, bf, bs);
            if (Q(i).cost <= Q(i).fs)
                Q(i).status = 1;
                R = [R; Q(i)];
            else
